  ...
  return (
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
      <div className="p-6 text-white">
        ...
        <p className="mt-4">Dashboard organizado e pronto para expans├úo.</p>
        <div className="mt-8 bg-slate-800 p-4 rounded">
          <p>Motoristas Ativos: {currentParams.activeDrivers}</p>
          <p>Receita Estimada: R$ {projections[0]?.grossRevenue?.toLocaleString?.() ?? 'ÔÇö'}</p>
        </div>
      </div>
    </Layout>
  );
...
export default App;
             ...
             <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl flex flex-col items-center">
                <h5 className="text-[11px] font-black text-yellow-500 uppercase mb-10 self-start">Distribui├º├úo de Aloca├º├úo de Verba</h5>
        ...
        {/* --- ABA 3: PARAMETRIZA├ç├âO --- */}
        {activeTab === 3 && (
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl animate-in fade-in duration-500">
            <h5 className="text-[11px] font-black uppercase text-yellow-500 mb-10 border-b border-slate-800 pb-4 flex items-center gap-2"><Sliders size={20}/> Painel de Par├ómetros Operacionais</h5>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
              {[
                { label: 'Frota Inicial', key: 'activeDrivers', min: 0, max: 500, step: 1, unit: ' condutores' },
                { label: 'Adi├º├úo Mensal Frota', key: 'driverAdditionMonthly', min: 0, max: 100, step: 1, unit: ' condutores' },
                { label: 'Corridas p/ Usu├írio', key: 'ridesPerUserMonth', min: 1, max: 10, step: 0.1, unit: ' rides/m├¬s' },
                { label: 'Tarifa M├®dio (R$)', key: 'avgFare', min: 10, max: 50, step: 0.5, unit: '' },
                { label: 'Crescimento Usu├írios (%)', key: 'userGrowth', min: 0, max: 30, step: 1, unit: '%' },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 ...
                 <div className="lg:col-span-1 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                    ...
                    <div className="space-y-2">
                       <p className="text-[9px] text-slate-400 uppercase leading-relaxed mb-4">Bonificamos a meritocracia: Quanto mais corridas o motorista realiza no m├¬s, menor ├® o Take Rate da plataforma.</p>
                       ...
                       <div className="mt-6 p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-lg flex items-start gap-3">
                          ...
                          <span className="text-[8px] text-yellow-500/80 uppercase font-bold leading-tight">M├®trica auditada mensalmente. Cashback creditado automaticamente na carteira do condutor.</span>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                    <h5 className="text-[11px] font-black text-yellow-500 uppercase mb-8 flex items-center gap-2"><UserCheck size={18}/> Gest├úo de Frota Alvo e GAP de Atendimento</h5>
                    <div className="overflow-x-auto">
                       <table className="w-full text-[10px] text-left border-collapse">
                          <thead className="bg-slate-950 text-slate-500 font-black uppercase border-b border-slate-800">
                             <tr><th className="p-5">M├¬s</th><th className="p-5 text-center">Frota Real</th><th className="p-5 text-center">Alvo (Min 50)</th><th className="p-5 text-center">GAP</th><th className="p-5 text-right">Status Cobertura</th></tr>
                          </thead>
                             ...
                             {results.map(r => {
                               ...
                               return (
                                 <tr key={r.month} className="hover:bg-slate-800/10">
                                    <td className="p-5 font-black text-white">M├èS {r.month}</td>
        ...
        {/* --- ABA 5: PROJE├ç├òES/ ESCALA --- */}
        {activeTab === 5 && (
          <div className="space-y-10 animate-in fade-in duration-500">
             {/* Comparativo de 1┬║ Semestre (Junho) */}
             {renderComparisonBlock("An├ílise Evolutiva: 1┬║ Semestre (Junho)", [6, 18, 30])}
             
             {/* Comparativo de 2┬║ Semestre (Dezembro) */}
             {renderComparisonBlock("An├ílise Evolutiva: 2┬║ Semestre (Dezembro)", [12, 24, 36])}
             ...
             <div className="bg-yellow-500/5 border border-yellow-500/20 p-4 rounded-xl flex items-start gap-3">
               ...
               <div>
                  <h6 className="text-xs font-black text-yellow-500 uppercase mb-1">Nota Metodol├│gica de Compara├º├úo</h6>
                  <p className="text-[10px] text-slate-400 leading-relaxed">
                     As setas e percentuais indicam a evolu├º├úo da m├®trica em rela├º├úo ao mesmo per├¡odo do ciclo anual anterior (Ex: M├¬s 18 comparado ao M├¬s 06). 
                     Os custos operacionais somam marketing, tecnologia, custos vari├íveis e custos fixos.
                  </p>
               </div>
             </div>
          </div>
        ...
        {activeTab === 6 && (
          <div className="space-y-6 animate-in fade-in duration-500">
            {(() => {
              // C├ílculos Espec├¡ficos para os Cards
              const breakEvenIndex = results.findIndex(r => r.netProfit > 0);
              const paybackIndex = results.findIndex(r => r.accumulatedProfit > 0);
              const breakEvenMonth = breakEvenIndex !== -1 ? `M├¬s ${results[breakEvenIndex].month}` : 'N├úo Atingido';
              const paybackMonth = paybackIndex !== -1 ? `M├¬s ${results[paybackIndex].month}` : 'Proje├º├úo > 36 Meses';
              ...
              // Estimativa de Churn acumulado (usu├írios perdidos te├│ricos)
              ...
              return (
                <div className="space-y-8">
                  {/* Se├º├úo 1: Milestones Financeiros */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className={`p-6 rounded-2xl border ${breakEvenIndex !== -1 ? 'bg-slate-900 border-green-500/50' : 'bg-slate-900 border-red-500/30'}`}>
                       <div className="flex justify-between items-start mb-4">
                          ...
                          <span className="text-[9px] font-black uppercase text-slate-500 bg-slate-950 px-2 py-1 rounded">Ponto de Equil├¡brio</span>
                       </div>
                       <div className="text-3xl font-black text-white">{breakEvenMonth}</div>
                       <div className="text-[10px] text-slate-400 mt-2">Momento onde a opera├º├úo deixa de queimar caixa mensalmente.</div>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl border bg-slate-900 border-slate-800">
                       ...
                       <div className="text-[10px] text-slate-400 mt-2">Resultado l├¡quido total somado ao longo de 3 anos de opera├º├úo.</div>
                    </div>
                  </div>

                  <h5 className="text-[11px] font-black text-yellow-500 uppercase flex items-center gap-2 border-b border-slate-800 pb-2">
                    <Activity size={16}/> Indicadores de Evolu├º├úo Operacional
                  </h5>

                  {/* Se├º├úo 2: Evolu├º├úo Operacional */}
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     {/* Card Churn */}
                     <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                        <div className="flex justify-between items-center mb-4">
                           <span className="text-[10px] font-black text-slate-500 uppercase">An├ílise de Churn</span>
                           <AlertTriangle size={16} className="text-red-400" />
                        </div>
                        <div className="text-2xl font-black text-white">{currentParams.churnRate}% <span className="text-xs text-slate-500 font-bold">/ m├¬s</span></div>
                        <div className="mt-2 text-[10px] text-slate-400">
                           Impacto estimado: <span className="text-red-400 font-bold">~{Math.round(estimatedChurnImpact)} usu├írios</span> perdidos no per├¡odo.
                        </div>
                     </div>
                     ...
                     <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                        <div className="flex justify-between items-center mb-4">
                           <span className="text-[10px] font-black text-slate-500 uppercase">Expans├úo de Frota</span>
                     ...
                     <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                        ...
                        <div className="mt-2 text-[10px] text-slate-400">
                           Penetra├º├úo sobre o SAM de {FRANCA_STATS.digitalUsers.toLocaleString()} usu├írios digitais.
                        </div>
                     </div>
             ...
             <div className="overflow-x-auto">
                <table className="w-full text-[10px] text-left border-collapse min-w-[1200px]">
                   ...
                   <tbody className="divide-y divide-slate-800/20 text-slate-300">
                      ...
                      <tr className="bg-yellow-500/10"><td className="p-4 sticky left-0 bg-slate-900 font-black text-yellow-500 uppercase">2. Receita L├¡quida TKX</td>{filteredDreResults.map(r=><td key={r.month} className="p-4 text-center font-bold text-yellow-500">{formatCurrency(r.takeRateRevenue).replace('R$', '')}</td>)}<td className="p-4 text-right font-black text-yellow-500">{formatCurrency(filteredDreResults.reduce((a,b)=>a+b.takeRateRevenue,0))}</td></tr>
                      ...
                      <tr>
                        <td className="p-4 sticky left-0 bg-slate-900 pl-8 text-slate-500">(-) Custos Vari├íveis / Banc├írios</td>
        ...
        {activeTab === 8 && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               {renderMetricCard('LTV / CAC', `${(lastResult.ltv / (lastResult.cac || 1)).toFixed(1)}x`, 'Efici├¬ncia de Aquisi├º├úo', <Activity size={18}/>, 'pos')}
               {renderMetricCard('Churn Rate', `${currentParams.churnRate}%`, 'Evas├úo de Usu├írios', <AlertTriangle size={18}/>, 'neg')}
               {renderMetricCard('ARPU TKX', formatCurrency(lastResult.takeRateRevenue / lastResult.users), 'Receita por Usu├írio', <Coins size={18}/>)}
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl h-[350px]">
               <h5 className="text-[11px] font-black text-yellow-500 uppercase mb-6">Equil├¡brio Frota vs Demanda</h5>
        ...
        {/* --- ABA 9: Cen├írios Detalhados --- */}
        {activeTab === 9 && (
          <div className="space-y-8 animate-in fade-in duration-500">
            {(() => {
               // Pr├®-calcular dados de todos os cen├írios
               const scenariosData = Object.values(ScenarioType).map(t => {
                  ...
                  // Pr├│s e Contras (L├│gica baseada no tipo)
                  let pros: string[] = [];
                  let cons: string[] = [];

                  if (t === ScenarioType.REALISTA) {
                     pros = ["Equil├¡brio Risco/Retorno", "Crescimento Sustent├ível", "Foco em Reten├º├úo"];
                     cons = ["Velocidade Moderada", "Exige Execu├º├úo Precisa"];
                  } else if (t === ScenarioType.PESSIMISTA) {
                     pros = ["Baixa Exposi├º├úo de Capital", "Maior Sobreviv├¬ncia em Crise", "Controle R├¡gido de Caixa"];
                     cons = ["ROI Longo Prazo", "Risco de Perda de Share", "Baixa atratividade para Motoristas"];
                  } else {
                     pros = ["Dom├¡nio R├ípido de Mercado", "Barreira de Entrada para Competidores", "Valuation Agressivo"];
               ...
               // Dados combinados para o gr├ífico
               ...
               return (
                  <>
                              ...
                              <div className="space-y-4">
                                 ...
                                 <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-800/50">
                                    <div>
                                       ...
                                       <div className="text-sm font-bold text-white">{s.metrics.paybackIdx !== -1 ? `M├¬s ${s.metrics.paybackIdx + 1}` : 'N/A'}</div>
                                    </div>
                                    <div>
                                       <span className="text-[9px] text-slate-500 uppercase">Break-even</span>
                                       <div className="text-sm font-bold text-white">{s.metrics.breakEvenIdx !== -1 ? `M├¬s ${s.metrics.breakEvenIdx + 1}` : 'N/A'}</div>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                       <span className="text-[9px] text-slate-500 uppercase">Cresc. Usu├írios</span>
                        ...
                        <div className="overflow-x-auto">
                           <table className="w-full text-[10px] text-left border-collapse">
                              <thead className="bg-slate-950 text-slate-500 uppercase font-black">
                                 <tr>
                                    <th className="p-3 w-32 sticky left-0 bg-slate-950 z-10">Cen├írio</th>
                                    <th className="p-3 text-center">M├®trica</th>
                                    {[6, 12, 18, 24, 30, 36].map(m => <th key={m} className="p-3 text-center bg-slate-950/50">M├¬s {m}</th>)}
                                 </tr>
                              </thead>
                                 ...
                                 {scenariosData.map((s) => (
                                    <React.Fragment key={s.type}>
                                       ...
                                       <tr className="hover:bg-slate-800/20 transition-colors bg-slate-950/20">
                                          <td className="p-3 text-center text-slate-500 font-bold uppercase text-[9px] border-b border-slate-800/50">Usu├írios</td>
                                          {[5, 11, 17, 23, 29, 35].map(idx => (
                     ...
                     {/* 4. Gr├ífico de Equity Comparativo */}
        ...
        {/* --- ABA 10: GERAL - DASHBOARD EXECUTIVO CONSOLIDADO (Vis├úo 360) --- */}
        {activeTab === 10 && (
          <div className="space-y-8 animate-in fade-in duration-500">
             {(() => {
                // C├ílculo de todos os cen├írios para o quadro comparativo consolidado
                ...
                return (
                   <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                         ...
                         <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative overflow-hidden">
                            <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest">Exposi├º├úo M├íxima Caixa</span>
                      ...
                      {/* 2. Matriz de Viabilidade Ano a Ano (Cora├º├úo do Dashboard) */}
                      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                         ...
                         <div className="overflow-x-auto">
                            <table className="w-full text-[10px] text-left border-collapse">
                               <thead className="bg-slate-950 text-slate-500 font-black uppercase border-b border-slate-800">
                                  <tr>
                                     <th className="p-4 w-32">Cen├írio</th>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                         {/* Sa├║de do Neg├│cio (Consolidado) */}
                         <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-2xl">
                            <h5 className="text-[11px] font-black text-yellow-500 uppercase mb-6 flex items-center gap-2"><Activity size={16}/> Sa├║de Operacional & M├®tricas (360┬║)</h5>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                               <div className="space-y-1">
                                  <span className="text-[9px] text-slate-500 uppercase font-bold">Efici├¬ncia Marketing</span>
                               ...
                               <div className="space-y-1">
                                  ...
                                  <div className="text-sm font-black text-yellow-500">{(lastResult.rides / lastResult.drivers).toFixed(0)} Rides/m├¬s</div>
                                  <div className="text-[9px] text-slate-400 mt-1">M├®dia por Driver</div>
                               </div>
                               <div className="space-y-1">
                                  <span className="text-[9px] text-slate-500 uppercase font-bold">Penetra├º├úo Mercado</span>
                         ...
                         <div className={`bg-slate-900 border-2 p-6 rounded-2xl flex flex-col justify-between ${isViable ? 'border-green-500/50 bg-green-500/5' : 'border-red-500/50 bg-red-500/5'}`}>
                            <div>
                               ...
                               <p className="text-[11px] leading-relaxed text-slate-300 font-medium">
                                  {isViable 
                                    ? "O cen├írio atual apresenta viabilidade econ├┤mica positiva. A opera├º├úo atinge o break-even dentro da janela de 36 meses e gera caixa livre sustent├ível no terceiro ano. Recomenda-se foco total na reten├º├úo de motoristas para manter a qualidade do servi├ºo durante a escala." 
                                    : "ATEN├ç├âO: O cen├írio atual apresenta alto risco de insolv├¬ncia. O volume projetado n├úo ├® suficiente para cobrir os custos fixos e vari├íveis acumulados. Recomenda-se revisar a estrat├®gia de aquisi├º├úo (CAC) ou aumentar o ticket m├®dio para garantir margem de contribui├º├úo positiva."}
                               </p>
                            </div>
                            <div className="mt-6 pt-4 border-t border-slate-800/50">
                               <span className="text-[9px] text-slate-500 uppercase font-bold">Melhor Cen├írio Identificado:</span>
        ...
        {/* --- ABA 11: VIS├âO 360┬║ (Mantida e enriquecida, mas foco foi na 10 conforme pedido) --- */}
        {activeTab === 11 && (
          <div className="space-y-6 animate-in fade-in duration-500">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group">
                   ...
                   <div className="text-[9px] text-slate-500 font-bold uppercase mt-1">M├║ltiplo 8x EBITDA</div>
                </div>
                ...
                {renderMetricCard('Burn M├íximo', formatCurrency(Math.min(...results.map(r=>r.netProfit))), 'Custo Operacional', <Zap size={18}/>, 'neg')}
             </div>
             <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
                <h5 className="text-[11px] font-black text-yellow-500 uppercase mb-6">Performance Anual Auditada</h5>
                <table className="w-full text-[10px] text-left">
                   <thead className="bg-slate-950 text-slate-500 uppercase font-black">
                      <tr><th className="p-4">Ano</th><th className="p-4">GMV Total</th><th className="p-4">Rec. TKX</th><th className="p-4">Profit L├¡quido</th><th className="p-4 text-right">Drivers/Users</th></tr>
                   </thead>
                   <tbody>
                      {audits.map(a => (
