...
def migrate(db_path: str = DB_PATH) -> None:
    ...
    cursor.execute(
        """
        INSERT INTO pricing_config (municipio, tarifa_base, valor_por_km, km_incluso, tarifa_minima, tech_fee_fixo, take_rate_pct)
        SELECT 'Franca', 4.00, 2.02, 0.00, 9.00, 0.70, 15.00
        WHERE NOT EXISTS (SELECT 1 FROM pricing_config WHERE municipio = 'Franca')
        """
    )
